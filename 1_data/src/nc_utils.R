...
nc_met_get <- function(nc_file,
                       var_name,
                       times = NULL){
  
  ncout = nc_open(nc_file)
  
  # met forecast dims ; position of dimensions following:
  time_pos = 1
  lon_pos = 3
  lat_pos = 2
  # each met forecast file should have just one forecast issue time
  
  cur_var = ncout$var[[var_name]]
  varsize = cur_var$varsize
  all_valid_times = c(ncdf_times(nc = ncout, timename = 'time'))

  n_dims = cur_var$ndims
   
  # return all values, and then filter
  all_out = array(ncvar_get(nc = ncout, varid = var_name), dim = varsize) %>%
    reshape2::melt(varnames = c("time", "lat", "lon")) %>%
    dplyr::select(-lon, -lat) %>%
    mutate(time = all_valid_times[time]) %>%
    rename(!!var_name := value) %>%
    as_tibble() %>% 
    group_by(time) %>% 
    mutate(hour = 0:(n()-1)) %>% ungroup() 
